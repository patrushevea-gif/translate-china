...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--provider",
        choices=["stub", "openai-compatible", "agentplatform"],
        default="stub",
...
def build_provider(args: argparse.Namespace) -> LLMProvider:
    if args.provider == "stub":
        return StubProvider()
    if args.provider == "agentplatform":
        api_key = os.getenv("AGENTPLATFORM_KEY", "").strip()
        if not api_key:
            raise SystemExit(
                "Environment variable AGENTPLATFORM_KEY is empty. "
                "Set it in your shell (export AGENTPLATFORM_KEY='...')."
            )
        provider = OpenAICompatibleProvider(
            base_url="https://api.agentplatform.ru/v1",
            api_key=api_key,
        )
        return FixedModelProvider(provider, model="openai/gpt-4o")
